...
def test_oom_after_convergence_does_not_deadlock():
    ...
    nxt, lo, hi = _advance_batch_search(current=20, lo=20, hi=21, max_bs=32, oom=True)
    assert nxt < 20
    assert hi == 20
    assert lo < 20
...
def test_oom_after_convergence_repeated_progresses():
    ...
    seen = [current]
    for _ in range(40):
        current, lo, hi = _advance_batch_search(current, lo, hi, max_bs=32, oom=True)
        seen.append(current)
        if current == 1:
            break
    assert seen[-1] == 1
...
def test_first_oom_bisects_to_half():
    nxt, lo, hi = _advance_batch_search(current=32, lo=0, hi=33, max_bs=32, oom=True)
    assert nxt == 16
    assert lo == 0
    assert hi == 32
...
def test_oom_cascade_reaches_one():
    ...
    for _ in range(20):
        current, lo, hi = _advance_batch_search(current, lo, hi, max_bs, oom=True)
        if current == 1:
            break
    assert current == 1
...
def test_success_with_no_ceiling_doubles():
    nxt, lo, hi = _advance_batch_search(current=4, lo=0, hi=33, max_bs=32, oom=False)
    assert nxt == 8
    assert lo == 4
...
def test_success_with_no_ceiling_clamps_to_max():
    nxt, _, _ = _advance_batch_search(current=24, lo=0, hi=33, max_bs=32, oom=False)
    assert nxt == 32


def test_success_with_known_ceiling_bisects_upward():
    nxt, lo, hi = _advance_batch_search(current=16, lo=0, hi=20, max_bs=32, oom=False)
    assert nxt == 18
    assert lo == 16
    assert hi == 20
...
def test_success_at_converged_bracket_holds_steady():
    ...
    nxt, lo, hi = _advance_batch_search(current=20, lo=20, hi=21, max_bs=32, oom=False)
    assert nxt == 20
    assert lo == 20
    assert hi == 21
...
def test_oom_strictly_lowers_hi():
    _, _, new_hi = _advance_batch_search(current=16, lo=4, hi=33, max_bs=32, oom=True)
    assert new_hi == 16


def test_success_strictly_raises_lo():
    _, new_lo, _ = _advance_batch_search(current=16, lo=4, hi=33, max_bs=32, oom=False)
    assert new_lo == 16
