...
def _run(tmp_path: Path, *, kept, labels, hoptimus_only=False, n_feat=None):
    ...
    X_raw = (np.arange(len(kept))[:, None] * 100 + np.arange(n_feat)[None, :]).astype(
        np.float32
    )
...
def test_khop_block_is_truncated_to_khop_dim(tmp_path):
    ...
    np.testing.assert_allclose(df[feature_cols].to_numpy(), X_raw[:, :KHOP_DIM])
...
def test_skips_existing_output_unless_overwrite(tmp_path):
    ...
    _, status = WORKER(
        (
            ...
            model_csv,
            np.arange(N_ROWS),
            np.zeros((N_ROWS, KHOP_DIM), dtype=np.float32),
            CLASSES,
