...
def _write_10x_h5(
    path: Path, barcodes, genes, dense_cells_by_genes, feature_types=None
):
    import h5py
    ...
    with h5py.File(path, "w") as f:
        ...
        g.create_dataset(
            "shape", data=np.array(M.shape, dtype="int64")
        )  # (genes, cells)
        ...
        fe.create_dataset(
            "feature_type", data=np.array([t.encode() for t in feature_types])
        )
    ...
    md = pd.DataFrame(
        {
            ...
            "height": [2.0] * 4,
            "prob_background": 0.0,
            "prob_neoplastic": [0.9, 0.1, 0.5, 0.2],
            "prob_inflammatory": 0.0,
            "prob_connective": 0.0,
            "prob_dead": 0.0,
            "prob_epithelial": 0.0,
        }
...
def test_import_affine_alignment_and_link(tmp_path):
    ...
    assert (
        a.obs["model_cell_id"] == "S1-" + a.obs["matched_box"].astype(int).astype(str)
    ).all()
