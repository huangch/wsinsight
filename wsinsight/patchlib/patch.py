"""Geometry helpers for turning binary masks into polygonal patch grids."""
...
def get_object_coordinates_within_polygon(
    object_centroids_arr: npt.NDArray[np.int_],
    half_patch_size: int,
    polygon: Polygon,
) -> npt.NDArray[np.int_]:
    ...
    object_centroids_poly = [Point(c) for c in object_centroids_arr]

    # Query which centroids are inside the polygon.
    tree = STRtree(object_centroids_poly)
    ...
    object_centroids_in_polygon = object_centroids_arr[centroid_indexes_in_polygon]
