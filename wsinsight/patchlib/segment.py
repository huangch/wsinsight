...
def segment_tissue(
    ...
    # if (1 <= red_threshold <= 255):
    #     # Define the HSV range for pink color
    #     lower_pink = np.array([red_threshold, 0, 0])
    #     upper_pink = np.array([255, 255, 255])
    #     # Threshold the HSV image to get only pink colors
    #     mask = cv.inRange(im_arr, lower_pink, upper_pink)
    #     # Bitwise-AND mask and original image
    #     im_arr = cv.bitwise_and(im_arr, im_arr, mask=mask)
