...
def patch(
    ...
    *,
    ...
    has_qupath = (
        qupath_measurement_detection_dir is not None
    ...
    if not has_model and not has_config and not has_qupath:
        raise click.UsageError(
            ...
            "or --qupath-measurement-detection-dir or --qupath-geojson-detection-dir "
    ...
    if has_model and has_qupath:
        raise click.UsageError(
            "--model is mutually exclusive with --qupath-measurement-detection-dir, "
            "--qupath-geojson-detection-dir, and --qupath-geojson-annotation-dir."
        )
    if has_config and has_qupath:
        raise click.UsageError(
            "--config/--model-path are mutually exclusive with --qupath-measurement-detection-dir, "
            "--qupath-geojson-detection-dir, and --qupath-geojson-annotation-dir."
        )
    ...
    if model_name is not None:
        ...
        stardist_normalization_pmin = (
            _config_dict["object_detection"]["normalization_pmin"]
            if object_detection == "stardist"
            else None
        )
        stardist_normalization_pmax = (
            _config_dict["object_detection"]["normalization_pmax"]
            if object_detection == "stardist"
            else None
        )
        halo_size_px = (
        ...
        ensure_output_directory(results_dir, "--results-dir")
    ...
    if (
        ...
        raise click.ClickException(
            "Only one of --overlap-ratio, --size-um, --size-px is allowed"
        )
    elif (
        nonzero_count == 1
        and object_based
